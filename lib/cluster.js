...
class AggregatorRegistry extends Registry {
	...
	clusterMetrics(callback) {
		...
		let workers, nWorkers;

		return new Promise((resolve, reject) => {
			pm2.list((err, descList) => {
				if (err) return reject(err);
				workers = descList.reduce((map, proc) => {
					// we only want cluster workers
					if (!proc.pm2_env || proc.pm2_env.exec_mode !== 'cluster_mode') {
						return map;
					}
					map[proc.pm_id] = {
						send: msg => {
							msg.id = proc.pm_id;
							// need empty function since pm2 is a dumb motherfucker
							pm2.sendDataToProcessId(msg, () => {});
						}
					};
					return map;
				}, {});
				nWorkers = Object.keys(workers).length;
				resolve();
			});
		}).then(
			() =>
				new Promise((resolve, reject) => {
					...
					for (const id in workers) workers[id].send(message);
				})
		);
	}
